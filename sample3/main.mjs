...
async function requestGeoCoderAPI(query) {
  const response = await axios({
    url: 'https://gleaming7moonlight29-eval-test.apigee.net/yahoo_map/geocode/V1/geoCoder',
    params: {
      query,
      output: 'json',
    },
...
async function main() {
  ...
  var strokeStyle = new Y.Style("00ff00", 10, 1.0);
  var fillStyle   = new Y.Style("00ff00", null, 1.0);
  var circle = new Y.Circle(new Y.LatLng(ansLat,ansLng), new Y.Size(1000, 1000), {
      unit:"km",
  ...
  document.getElementById('search').onsubmit = () => {
    ...
    (async () => {
      const coordinates = ansLat + ","+  ansLng + "%20" + center.Lat +',' +center.Lng;
